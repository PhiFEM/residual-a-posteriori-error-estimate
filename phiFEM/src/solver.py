...
from phiFEM.src.mesh_scripts import plot_mesh_tags, compute_outward_normal
...
class PhiFEMSolver:
    ...
    def compute_tags(self, padding=False, plot=False):
        ...
        if plot:
            ...
            figure, ax = plt.subplots()
            plot_mesh_tags(working_mesh, working_cells_tags, ax=ax, display_indices=False, expression_levelset=self.levelset.expression)
            plt.savefig(f"./output_phiFEM/test_cells_{str(self.i).zfill(2)}.svg", format="svg", dpi=2400)
